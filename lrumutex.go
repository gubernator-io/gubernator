...
// LRUMutexCache is a mutex protected LRU cache that supports expiration and is thread-safe
type LRUMutexCache struct {
...
var _ Cache = &LRUMutexCache{}

// NewLRUMutexCache creates a new Cache with a maximum size.
func NewLRUMutexCache(maxSize int) *LRUMutexCache {
	setter.SetDefault(&maxSize, 50_000)

	return &LRUMutexCache{
...
func (c *LRUMutexCache) Each() chan *CacheItem {
...
func (c *LRUMutexCache) Add(item *CacheItem) bool {
	return false
}
...
func (c *LRUMutexCache) AddIfNotPresent(item *CacheItem) bool {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *LRUMutexCache) GetItem(key string) (item *CacheItem, ok bool) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *LRUMutexCache) Remove(key string) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *LRUMutexCache) removeOldest() {
	ele := c.ll.Back()
	if ele != nil {
		entry := ele.Value.(*CacheItem)
...
func (c *LRUMutexCache) removeElement(e *list.Element) {
	c.ll.Remove(e)
...
func (c *LRUMutexCache) Size() int64 {
	return atomic.LoadInt64(&c.cacheLen)
}

func (c *LRUMutexCache) Close() error {
	c.cache = nil
	c.ll = nil
	c.cacheLen = 0
	return nil
...
func (c *LRUMutexCache) Stats() CacheStats {
	c.mu.Lock()
	defer func() {
		c.stats = CacheStats{}
		c.mu.Unlock()
