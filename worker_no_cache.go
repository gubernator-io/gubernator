...
type WorkerNoCache struct {
	conf  Config
	cache Cache
}

// NewWorkerNoCache creates a new singleton using the otter cache.
func NewWorkerNoCache(conf Config) *WorkerNoCache {
	cache, err := NewNoCache(conf.CacheSize)
	if err != nil {
		panic(err)
	}
	return &WorkerNoCache{
		cache: cache,
		conf:  conf,
	}
}

func (w *WorkerNoCache) GetRateLimit(ctx context.Context, req *RateLimitReq, state RateLimitReqState) (*RateLimitResp, error) {
...
func (w *WorkerNoCache) Store(ctx context.Context) error {
...
func (w *WorkerNoCache) Close() error {
